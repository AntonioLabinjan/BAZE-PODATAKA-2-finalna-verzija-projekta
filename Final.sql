...
# ALTERNATIVNO (naprednija i ljepša verzija)
SELECT Z.id_odjel, O.naziv, SUM(ZA.vrijednost) AS ukupna_vrijednost_zapljena
FROM Slucaj S
JOIN Zapljene ZA ON S.Id = ZA.id_slucaj
JOIN Zaposlenik Z ON S.id_voditelj= Z.Id
JOIN Odjeli O ON Z.id_odjel = O.id
...
# ALTERNATIVNO 
CREATE VIEW pogled_policijskih_sluzbenika_2 AS
SELECT
    O.Id AS zaposlenik_id,
    O.Ime_Prezime AS ime_prezime_osobe,
    O.datum_rodenja AS datum_rodenja_osobe,
    TIMESTAMPDIFF(YEAR, Z.datum_zaposlenja, CURRENT_DATE) AS Godine_Staza,
    TIMESTAMPDIFF (YEAR, O.datum_rodenja, CURRENT_DATE) AS Trenutna_Dob,
    CASE
        WHEN Z.datum_izlaska_iz_sluzbe IS NOT NULL AND Z.Datum_izlaska_iz_sluzbe <= CURRENT_DATE THEN 'Da'
        ELSE 'Ne'
    END AS Umirovljen
...
#### TEST ######
SELECT * FROM pogled_policijskih_sluzbenika;
SELECT * FROM pogled_policijskih_sluzbenika_2;
...
SELECT slucaj.id as SLUCAJ, kaznjiva_djela.id as KD, pas.id as PAS, pas.id_kaznjivo_djelo AS DJELO_PSA FROM kaznjiva_djela INNER JOIN kaznjiva_djela_u_slucaju INNER JOIN slucaj INNER JOIN pas;
# 10)Triger koji premješta završene slučajeve iz tablice slučaj u tablicu arhiva # PRETVORENO JE U PROCEDURU JER JE TO JAKO VAŽAN DIO POLICIJSKOG RADA I NE ŽELIMO DA SE ODRAĐUJE AUTOMATSKI
######################### A.L. #############################
# Privremeno uklonimo vanjske ključeve
...
BEGIN
	...
	IF slucaj_status IS NULL THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Slučaj s navedenim ID-om ne postoji.';
	ELSEIF slucaj_status = 'arhiviran' THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Slučaj je već arhiviran.';
    ELSEIF slucaj_status <> 'riješen' AND slucaj_status <> 'arhiviran'  THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Slučaj nije riješen i ne može biti premješten u arhivu.';
    ELSE
        # Promijeni status slučaja u arhiviran
        UPDATE Slucaj SET status = 'arhiviran' WHERE id = p_slucaj_id;

        # Premjesti slučaj iz Slucaj u Arhiva
...
SELECT id, status FROM slucaj;
SELECT * FROM Arhiva;
CALL Oznaci_Slucaj_Arhiva(4);
DROP TABLE Arhiva;
...
UPDATE pas  SET godina_rođenja = 1923 WHERE id = 7;
# 20) Triger koji će ograničiti da isti zaposlenik ne smije istovremeno voditi više od 5 aktivnih slučajeva kako ne bi bio preopterećen
########################## A.L. #######################
DELIMITER //
...
# u9)upit koji će za svakog zaposlenika pozvati funkciju uspješnosti i vratiti rezultat, osim ako nije vodio slučajeve, onda će vratiti odgovarajuću obavijest
    CREATE VIEW zaposlenici_uspješnost AS SELECT
...
SELECT * FROM zaposlenici_uspješnost;
...
################## GARGANTUOZNA PROCEDURA #########################
DROP PROCEDURE ProvjeriIstekZatvorskeKazne;
CALL ProvjeriIstekZatvorskeKazne();
SELECT * FROM osoba O INNER JOIN slucaj S ON S.id_pocinitelj = O.id;
DELIMITER //
...
BEGIN
    ...
    read_loop: LOOP
        ...
        SET danas = NOW();
        IF DATE_ADD(datum_zavrsetka_slucaja, INTERVAL ukupna_kazna YEAR) <= danas THEN
            ...
            WHERE Id = osoba_id;
        ELSE
            # Kazna nije istekla, postavljamo obavijest da je u tijeku
            UPDATE Osoba
            SET obavijest = 'Kazna je u tijeku'
            WHERE Id = osoba_id;
        END IF;
    END LOOP;
